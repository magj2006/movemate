...
module movemate::box {
    ...
    struct Boxes<T: store> has key, store {
        // boxes: IterableTable<address, vector<T>>
        boxes: SimpleMap<address, vector<T>>
    }
    ...
    public entry fun init<T: store>(sender: &signer) {
        move_to(sender, Boxes<T> {
            // boxes: iterable_table::new()
            boxes: simple_map::create()
        });
    }
    ...
    public entry fun box<T: store>(sender: address, recipient: address, obj_in: T) acquires Boxes {
        ...
        // let box = iterable_table::borrow_mut(boxes, recipient);
        let box = simple_map::borrow_mut(boxes, &recipient);
    ...
    public entry fun unbox<T: store>(sender: address, recipient: &signer, index: u64): T acquires Boxes {
        let boxes = &mut borrow_global_mut<Boxes<T>>(sender).boxes;
        // let box = iterable_table::borrow_mut(boxes, signer::address_of(recipient));
        let box = simple_map::borrow_mut(boxes, &signer::address_of(recipient));
    ...
    public entry fun box_private<T: store>(sender: &signer, recipient: address, obj_in: T) acquires PrivateBoxes {
        ...
        if (exists<PrivateBoxes<T>>(sender_address)) {
            ...
            // let boxes = iterable_table::new<address, vector<T>>();
            // iterable_table::add(&mut boxes, recipient, vector::singleton(obj_in));

            let boxes = simple_map::create<address, vector<T>>();
            simple_map::add(&mut boxes, recipient, vector::singleton(obj_in));
            move_to(sender, Boxes<T> {
                boxes
            });
        }
